...
void set_sign(s21_decimal *dst) {
    dst->bits[3] = setbit(dst->bits[3], 31);
}

void reset_sign(s21_decimal *dst) {
    dst->bits[3] = resetbit(dst->bits[3], 31);
}
...
int shift_left(s21_decimal *dec, int shift) {
    ...
    if (highest_bit + shift > 95) {
        ...
        for (int i = 0; i < shift; i++) {
            ...
            if (last_low_byte_bit)
                decsetbit(dec, 32);
            if (last_mid_byte_bit)
                decsetbit(dec, 64);
        }
    }
    if (get_sign(*dec) && res)
...
s21_decimal binary_div(s21_decimal dec1, s21_decimal dec2, s21_decimal *reminder, int *fail) {
    nuller(reminder);
    s21_decimal result = {{0, 0, 0, 0}};
    int result_scale = 0;
    *fail = handle_scales_equality(&dec1, &dec2, &result_scale);
    for (int i = get_highest_bit(dec1); i >= 0; i--) {
        if (deccheckbit(dec1, i))
            decsetbit(reminder, 0);
        if (s21_is_greater_or_equal(*reminder, dec2)) {
            *fail = sub_without_scale(*reminder, dec2, reminder);
            if (i != 0)
                *fail = shift_left(reminder, 1);
            if (deccheckbit(dec1, i - 1))
                decsetbit(reminder, 0);
...
int handle_scales_equality(s21_decimal *dec1, s21_decimal *dec2, int *final_scale) {
...
void sub_only_bits(s21_decimal dec1, s21_decimal dec2, \
                   s21_decimal *result) {
    nuller(result);
