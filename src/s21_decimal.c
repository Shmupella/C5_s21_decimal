...
int s21_from_decimal_to_float(s21_decimal src, float *dst) {
    ...
    if (dst) {
        for (int i = 0; i < 96; i++)
            if (deccheckbit(src, i))
                tmp += pow(2, i);
        for (int i = 16; i < 24; i++)
            if (checkbit(src.bits[3], i))
                scale = setbit(scale, i - 16);
        if (scale > 0) {
...
int s21_from_decimal_to_int(s21_decimal src, int *dst) {
    *dst = 0;
    ...
    if (res != OK || src.bits[1] || src.bits[2] ||
...
int s21_is_equal(s21_decimal dec1, s21_decimal dec2) {
    ...
    if (dec1.bits[0] == 0 && dec1.bits[1] == 0 && dec1.bits[2]\
     == 0 && dec2.bits[0] == 0 && dec2.bits[1] == 0 && dec2.bits[2] == 0) {
        result = 1;
...
int s21_mul(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
    ...
    int sign = 0;
    if ((get_sign(value_1) && !get_sign(value_2)) || (!get_sign(value_1)
    && get_sign(value_2)))
        sign = 1;
    value_1.bits[3] = 0;
    value_2.bits[3] = 0;
...
int s21_div(s21_decimal dec1, s21_decimal dec2, s21_decimal *result) {
    ...
    if (s21_is_equal(dec2, zero)) {
        ...
        *result = dec_int_part;
        ...
        if (get_sign(dec1) != get_sign(dec2))
            set_sign(result);
        if (s21_is_equal(*result, zero) && !s21_is_equal(dec1, zero))
            ret_val = 2;
    }
    return ret_val;
}
